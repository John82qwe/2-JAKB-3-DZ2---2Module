...
public class HomeWork2 {
    ...
    public static void validateLogin(String login) throws LoginValidationException{
        ...
        boolean ContainsSmallLater = false;
        boolean ContainsBigLater = false;
        boolean ContainsNumbers = false;
        boolean ContainsCheck = false;   // _
        boolean error = false;
        for (int i = 0; i < login.length(); i++) {
            if (login.charAt(i) >= 'a' && login.charAt(i) <= 'z') {
                ContainsSmallLater = true;
            }
            else if (login.charAt(i) >= 'A' && login.charAt(i) <= 'Z') {
                ContainsBigLater = true;
            }
        ...
        if (error == true || ContainsSmallLater == false || ContainsBigLater == false || ContainsNumbers == false || ContainsCheck == false){
            throw new LoginValidationException();
        }
